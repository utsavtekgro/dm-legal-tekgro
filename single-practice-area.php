<?php
/**
 * Single Practice Area — native CPT permalink view (/practice-areas/{slug}/).
 *
 * Renders the same rich detail layout as page-practice-area.php?slug=, but
 * driven by the current practice_area post via the main query. Routed here by
 * dm_legal_practice_area_single_template() (see inc/post-types.php).
 *
 * @package DM_Legal
 * @since   1.0.0
 */
...
$dm_pa_post = get_queried_object();

if ( ! $dm_pa_post instanceof WP_Post ) {
	status_header( 404 );
	get_footer();
	return;
}

$service  = dm_legal_practice_area_fields( $dm_pa_post );
$slug     = $dm_pa_post->post_name;
$dm_pa_id = (int) $dm_pa_post->ID;

// Hero content is editable per area via the "Detail Hero" metabox.
$dm_pa_hero = dm_legal_pa_hero_args(
	array(
...
$actionsData = dm_legal_pa_actions( $dm_pa_id, $actionsDataBySlug[ $slug ] ?? null );
?>
...
<!-- ============ Process ============ -->
<?php $dm_pa_steps = dm_legal_pa_steps( $dm_pa_id, $steps ); ?>
...
<!-- ============ FAQ ============ -->
<?php
$dm_pa_faq = dm_legal_pa_faq_args(
	array(
