<?php
/**
 * Site footer + full map card + mobile bottom nav + "under development" modal.
 * Ported 1:1 from the legacy static footer.php. Scripts are enqueued via
 * inc/enqueue.php instead of being hard-coded, and wp_footer() is fired.
 *
 * @package DM_Legal
 * @since   1.0.0
 */
...
global $officeInfo, $usefulLinks, $blogs;
$officeInfo  = $officeInfo ?? array();
$usefulLinks = $usefulLinks ?? array();
$blogs       = $blogs ?? array();

// The contact page shows its own office/map block, so skip the CTA there.
if ( 'contact' !== dm_legal_current_slug() ) {
	include DM_LEGAL_DIR . '/inc/parts/contact-footer.php';
}
?>
 ...
 <div class="full-map-card">
    ...
    <div class="full-map-card__panel">
      ...
      <div class="full-map-card__grid">
        ...
        <?php foreach ( ( $officeInfo['phoneNumbers'] ?? array() ) as $phone ) : ?>
          <div class="full-map-card__item">
  ...
  <div class="content-width footer-main">
    <div class="footer-top mt-5">
      <div class="footer-top__brand">
        ...
        <p class="body-text fw-bold">DM LEGAL AUSTRALIA | ABN 32 626 700 981</p>
      </div>
...
<script id="blogs-data" type="application/json"><?= wp_json_encode( array_map( function ( $b ) {
    ...
    return array(
        ...
        'image'    => url( $b['image'] ),
        'author'   => $b['author'],
        'link'     => url( 'blog-detail.php?slug=' . $slug ),
    );
